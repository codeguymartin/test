...
AreaMinRED=5 #pixel area is (width x length of pixels). This is min area of blobs that are kept, and considered Objects
...
aspect_ratio_rot_box_RED_MIN=0.005
aspect_ratio_rot_box_RED_MAX=1111.5
...
for file in FILES: #MAIN FOR LOOP
    ...
    if file.endswith('golf_scores_N2_marked_TabletInBox.jpg'):   #  THESE ARE THE RAW IMAGES..... for binary then it would be if file.endswith('_binary.tif'):
        print file
...
######################### RED - START
        ...
        #cv2.resize(maskRED, (0, 0), fx=0.5, fy=0.5)
        cv2.imwrite(dir_pathOUT+'/mask.jpg', maskRED)
        cv2.imwrite(dir_pathOUT + '/image_contours.jpg', im)
        ...
        cv2.imwrite(dir_pathOUT + '/image_min_rect.jpg', im_copy)
